const filterNodes = {
  ...
  closeDrawer: document.querySelector('[data-closeDrawer]')
}
...
class CollectionFilters {
  ...
  */
  bindEvents() {
    ...
    filterNodes.closeDrawer.addEventListener('click', this.toggleFilterDrawer.bind(this));
  ...
  _createQueryString(form){
    // const formData = Array.from(new FormData(form)).filter(([k, v])=> { return v });
    const formData = new FormData(form);
    const priceGTE = form.querySelector('[name="filter.v.price.gte"]').min;
    const priceLTE = form.querySelector('[name="filter.v.price.lte"]').max;
    
    if(formData.get('filter.v.price.gte') == priceGTE){
      formData.delete('filter.v.price.gte');
    }if(formData.get('filter.v.price.lte') == priceLTE){
  ...
  */
  ...
  renderProductGrid(type, grid, banner) {
    ...
    let drawerOpen = filterNodes.parent.classList.contains('filter-active');
    if(drawerOpen) filterNodes.closeDrawer.dispatchEvent(new Event('click'));
  }
